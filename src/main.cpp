...
unsigned int nStakeMinAge = 60 * 60 * 24 * 1;	// minimum age for coin age: 15d
unsigned int nStakeMaxAge = 60 * 60 * 24 * 30;	// stake age of full weight: 30d
unsigned int nStakeTargetSpacing = 60 * 5;			// 5 mins block spacing

static const int64 nTargetTimespan = 60 * 10;  // every 10 minutes
static const int64 nTargetSpacingWorkMax = nStakeTargetSpacing; // 2 * nStakeTargetSpacing; // 20 mins  (replaces nTargetSpacing from PoW)

int64 nChainStartTime = 1422145922; //1399076060;
int nCoinbaseMaturity = 15;  // Blocks till reward is mature
...
const string strMessageMagic = "BANX Signed Message:\n";
...
const int YEARLY_BLOCKCOUNT = 365 * 24 * 12; //1051200;	// 365 * 2880
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
	...
	if(nHeight <= 100)
    {
        nSubsidy = 120000 * COIN; // 11999808 coins + 105 per year  Close enough to 12 millions not to matter.
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in BANX
    bool fStrictPayToScriptHash = true; // Always active in BANX
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    ...
    if (!CheckStakeModifierCheckpoints(pindexNew->nHeight, pindexNew->nStakeModifierChecksum))
        return error("AddToBlockIndex() : Rejected by stake modifier checkpoint height=%d, modifier (checkpoint)=0x%016"PRI64x, pindexNew->nHeight, pindexNew->nStakeModifierChecksum);
...
bool CBlock::AcceptBlock()
{
    ...
    BOOST_FOREACH(const CTransaction& tx, vtx)
    {
        ...
        // Adriano 2014-04-19
        if(nHeight > 28647){
            static const CBitcoinAddress lostWallet ("CKGK6MFmBkreG7k5sU8gDEJNVJ57QZtN3H");  // TODO: remove this?
            for (unsigned int i = 0; i < tx.vin.size(); i++){
                uint256 hashBlock;
                CTransaction txPrev;
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "BANX", CClientUIInterface::MSG_WARNING);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Banxshares launches!";
        CTransaction txNew;
        ...
        block.nVersion = 1;
        block.nTime    = 1422145922;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1397106;
        ...
        assert(block.hashMerkleRoot == uint256("0x594b123ad204bad59ae9d3068ebaf1f914b3730a8aad2fb6632aa89883cb9640")); //0x61f9e7e3a52be28f0ac7fb759c325a694dc85bc4fc97d7c4300a2bd4d5d0bbcd"));
